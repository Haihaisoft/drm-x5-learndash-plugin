...
/** Coordinates login, LearnDash authorization and DRM-X license delivery. */
class DRMX5_LD_License_Controller {
    public static function handle() {
        nocache_headers();
        $settings = wp_parse_args(get_option('drmx5_ld_settings', array()), DRMX5_LD_Settings::defaults());
        ...
        if (DRMX5_LD_Request::is_new_request()) {
            $request = DRMX5_LD_Request::capture();
        } elseif ($resume_token !== '') {
            $saved = get_transient('drmx5_ld_request_' . $resume_token);
            $request = is_array($saved) ? DRMX5_LD_Request::restore($saved) : null;
        } else {
            $request = null;
        }

        if (!$request) {
            self::error_page(__('No pending DRM-X license request was found. Please open the protected content again.', 'drmx5-learndash'));
        }
        ...
        if (!is_user_logged_in()) {
            ...
            set_transient('drmx5_ld_request_' . $token, $request->export(), 10 * MINUTE_IN_SECONDS);
        ...
        if (empty($settings['admin_email']) || empty($settings['webservice_auth']) || empty($settings['group_id'])) {
            self::error_page(__('DRM-X AdminEmail, WebServiceAuthStr or GroupID has not been configured by the site administrator.', 'drmx5-learndash'));
        }
        if (!function_exists('sfwd_lms_has_access')) {
            self::error_page(__('LearnDash is not installed or activated.', 'drmx5-learndash'));
        }

        $user = wp_get_current_user();
        $course = DRMX5_LD_Course_Authorizer::get_accessible_course($request->get('yourproductid'), $user->ID);
        if (!$course) {
            self::error_page(__('Your account is not actively enrolled in any course assigned to this protected content.', 'drmx5-learndash'));
        }

        if ($rights_mode === 'fixed') {
            $rights_id = trim((string) $settings['fixed_rights_id']);
            if ($rights_id === '') {
                self::error_page(__('Fixed RightsID mode is enabled, but no Fixed RightsID has been configured.', 'drmx5-learndash'));
            }
            if (empty($course['start']) || empty($course['end']) || $course['end'] < $course['start']) {
                self::error_page(__('The matched LearnDash course dates are invalid and cannot be used to update the DRM-X permission.', 'drmx5-learndash'));
            }
        } else {
        ...
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('DRM-X 5.0 LearnDash: ' . $exception->getMessage());
            }
            self::error_page(__('The DRM-X service request failed. Please contact the site administrator.', 'drmx5-learndash'));
        }

        if (stripos($license, 'license_div_drm-x5') === false) {
            $details = trim($message . ' ' . wp_strip_all_tags($license));
            self::error_page(sprintf(__('DRM-X rejected the license request: %s', 'drmx5-learndash'), $details));
        }

        if ($resume_token !== '') {
            delete_transient('drmx5_ld_request_' . $resume_token);
        }
        self::success_page($license, $message, $request->get('return_url'));
    }

    public static function endpoint_url() {
        return plugins_url('licstore5.php', DRMX5_LD_FILE);
    }

    private static function error_page($message) {
        header('Content-Type: text/html; charset=' . get_bloginfo('charset'));
        $title = __('Unable to obtain DRM-X license', 'drmx5-learndash');
    ...
    private static function success_page($license, $message, $return_url) {
        ...
        echo '<button type="button" id="drmx5-open">' . esc_html__('Open protected content', 'drmx5-learndash') . '</button></main>';
