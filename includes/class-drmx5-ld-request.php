    ...
    public function validate($rights_mode) {
        foreach (array('profileid', 'clientinfo', 'yourproductid') as $required) {
            if ($this->get($required) === '') {
                return __('The DRM-X request is missing a required parameter.', 'drmx5-learndash');
            }
        }
        if ($rights_mode === 'post' && (!$this->rightsid_from_post || $this->get('rightsid') === '')) {
            return __('ZJGet did not POST a RightsID. Check the permission mode and DRM-X License Profile.', 'drmx5-learndash');
        }
        if (preg_match('~^(?:javascript|data|vbscript):~i', $this->get('return_url'))) {
            return __('The DRM-X return URL is invalid.', 'drmx5-learndash');
        }
        return '';
    }
}
