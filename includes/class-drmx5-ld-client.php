...
class DRMX5_LD_Client {
    ...
    public function __construct($admin_email, $auth_string, $group_id, $service_area = 'international') {
        if (!extension_loaded('soap')) {
            throw new RuntimeException(__('The PHP SOAP extension is not enabled on this WordPress server.', 'drmx5-learndash'));
        }
    ...
    public function ensure_user_exists(WP_User $user) {
        ...
        if (strcasecmp($exists, 'False') !== 0 && $exists !== '0') {
            throw new RuntimeException(sprintf(
                __('DRM-X could not check the user account. Service response: %s', 'drmx5-learndash'),
                $exists
            ));
        }
        ...
        if ($result !== '1') {
            throw new RuntimeException(sprintf(
                __('DRM-X could not create the user account. Service response: %s', 'drmx5-learndash'),
                $result
            ));
        }
        return false;
    ...
    public function get_user_block_reason($username) {
        ...
        if (!isset($response->CheckUserIsRevokedResult)) {
            throw new RuntimeException(__('DRM-X did not return a valid user revocation status.', 'drmx5-learndash'));
        }
        if ($this->is_true($response->CheckUserIsRevokedResult)) {
            return __('Your account has been revoked. You cannot obtain a license. Please contact the site administrator.', 'drmx5-learndash');
        }
        ...
        if (!isset($response->GetUserDetailsResult) || !is_object($response->GetUserDetailsResult) ||
            ...
            throw new RuntimeException(__('DRM-X did not return valid user details.', 'drmx5-learndash'));
        }
        if ($this->is_true($response->GetUserDetailsResult->IsLockedOut)) {
            return __('Your account has been locked. You cannot obtain a license. Please contact the site administrator.', 'drmx5-learndash');
        }
        return '';
    }
    ...
    public function update_rights($rights_id, $watermark, $course_start, $course_end) {
        ...
        $response = $this->soap->__soapCall('UpdateRightWithDisableVirtualMachine', array('parameters' => array(
            ...
            'WebServiceAuthStr' => $this->auth_string,
            'RightsID' => $rights_id,
            'Description' => 'WordPress LearnDash DRM-X 5.0',
            'PlayCount' => '-1',
        ...
        if ($result !== '1') {
            throw new RuntimeException(sprintf(
                __('DRM-X could not update the permission. Service response: %s', 'drmx5-learndash'),
                $result
            ));
        }
    }

    public function get_license(DRMX5_LD_Request $request, WP_User $user, $rights_id) {
        $full_name = trim($user->first_name . ' ' . $user->last_name);
        if ($full_name === '') {
            $full_name = $user->display_name ?: $user->user_login;
        }
